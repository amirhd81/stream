...
def download_video(url, height):
    ...
    run(f"yt-dlp -4 -N 8 --concurrent-fragments 8    --add-header 'Referer: https://ln5.sync.com/'  --downloader [m3u8]aria2c --downloader-args aria2c:-x:16:-k:1M:-4 -f 480p -o '{os.path.join(DOWNLOAD_DIR, "video.%(ext)s")}' '{url}'")
