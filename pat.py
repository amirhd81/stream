...
def download_video(url, height):
    ...
    run(f"yt-dlp -4 --add-header 'Referer: https://www.whoreshub.com/videos/86424/chloe-lamour-and-mina-window-cleaner-studs-dp-slutty-stepmother-and-daughter-gp2367/' --add-header 'Host: https://www.whoreshub.com/' --downloader [m3u8]aria2c --downloader-args aria2c:-x:16:-k:1M:-4  --merge-output-format 'mp4' -o '{os.path.join(DOWNLOAD_DIR, "video.%(ext)s")}' '{url}'")
